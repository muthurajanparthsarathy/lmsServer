...
const exerciseSchema = new mongoose.Schema(
  {
    ...
    // keyed by sectionId. Non-section assessments leave this empty and use
    // the aggregate `customDistribution` above. Stored as Mixed so Mongoose
    // doesn't strip the dynamic section-id keys — client already validates
    // the { [sid]: { easy/medium/hard: { scratch/ai/thirdParty: number } } }
...
// ─── STUDENT FILE PROGRESS SCHEMA ────────────────────────────────────────────
const studentFileProgressSchema = new mongoose.Schema(
  {
    studentId: { type: String, required: true },
    fileName:  { type: String, default: "" },
    highestPageReached: { type: Number, default: 0 },
    totalPages: { type: Number, default: 0 },
    // Which page/slide numbers have had their MCQ attended (one entry per page)
    attemptedPages: [{ type: Number }],
    // Lightweight attendance log — NO answer details (answers stored separately in lms-users)
    markerAnswers: [
      {
    ...
    // Progress % = (attended MCQ pages / total MCQ pages on this file) × 100
...
const fileSchema = new mongoose.Schema(
  {
    ...
    studentFileProgress: [studentFileProgressSchema],
